  ...
  const seasonalData = useMemo(() => {
    ...
    return Array.from(byMonth.entries())
      ...
      .map(([month, vals]) => ({
        ...
        saison: month >= 10 || month <= 2 ? "Sèche" : "Pluies",
      }));
  }, [airQuality]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-secondary text-sm mt-1">
            Analyse des corrélations climat / qualité de l&apos;air
          </p>
        </div>
        <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <KPICard
          title="Ville la plus polluée"
          value={stats.worst}
          icon={<TrendingUp className="w-6 h-6" />}
          color="#EF4444"
        />
        <KPICard
          title="Ville la moins polluée"
          value={stats.best}
      ...
      <div className="bg-surface rounded-2xl border border-border p-6">
        <h3 className="text-sm font-semibold text-text mb-4">
          Climat vs Qualité de l&apos;air — 30 derniers jours
        </h3>
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={climateVsAQI} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
            ...
            <Bar yAxisId="right" dataKey="temperature" name="Temp (°C)" fill="#F9731660" radius={[4, 4, 0, 0]} />
            <Line yAxisId="right" type="monotone" dataKey="vent" name="Vent (km/h)" stroke="#3B82F6" strokeWidth={1.5} dot={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-surface rounded-2xl border border-border p-6">
          <h3 className="text-sm font-semibold text-text mb-4">
            Analyse saisonnière — AQI moyen par mois
          </h3>
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={seasonalData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 12, fontSize: 13 }} />
              <Bar dataKey="aqi" name="AQI moyen" radius={[6, 6, 0, 0]}>
                {seasonalData.map((entry, i) => (
                  <rect key={i} fill={entry.saison === "Sèche" ? "#F59E0B" : "#3B82F6"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          <div className="flex items-center gap-4 mt-3 justify-center text-xs text-text-secondary">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded bg-[#F59E0B]" /> Saison sèche
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded bg-[#3B82F6]" /> Saison des pluies
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-2xl border border-border p-6">
        ...
        <CorrelationMatrix data={climateVsAQI} />
      </div>
    </div>
  );
}

function CorrelationMatrix({ data }: { data: { aqi: number; temperature: number; vent: number; precipitation: number }[] }) {
  const vars = ["aqi", "temperature", "vent", "precipitation"] as const;
  const labels: Record<string, string> = { aqi: "AQI", temperature: "Température", vent: "Vent", precipitation: "Précipitations" };
