...
const DashboardMap = dynamic(() => import("@/components/map/DashboardMap"), {
  ssr: false,
  loading: () => <div className="h-[calc(100vh-8rem)] animate-pulse bg-surface rounded-2xl" />,
});
...
export default function MapPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-text">Carte interactive</h1>
        <p className="text-text-secondary text-sm mt-1">
          Qualité de l&apos;air en temps réel sur les 40 villes du Cameroun
        </p>
      </div>
      <DashboardMap cities={mapCities} height="calc(100vh - 12rem)" />
    </div>
  );
}
